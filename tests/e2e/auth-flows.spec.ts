...
test("account page exposes signed-out auth actions and manual account entry", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Create a manual account" }))
    .toHaveAttribute("href", "/account/manual?next=%2Fscreeners");
});
...
test("account page shows profile-required callback notice", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Retry with Google" }))
